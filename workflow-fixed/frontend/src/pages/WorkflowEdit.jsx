...
export default function WorkflowEdit() {
  ...
  const [form, setForm] = useState({ name: '', description: '', version: '1.0.0', status: 'draft', tags: '', inputSchema: '{}' });
  ...
  useEffect(() => {
    workflowAPI.getById(id).then(res => {
      const wf = res.data;
      setForm({
        name: wf.name || '', description: wf.description || '', version: wf.version || '1.0.0',
        status: wf.status || 'draft', tags: (wf.tags || []).join(', '),
        inputSchema: JSON.stringify(wf.inputSchema || {}, null, 2),
      });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    let inputSchema = {};
    try { inputSchema = JSON.parse(form.inputSchema); setSchemaError(''); }
    catch { setSchemaError('Invalid JSON'); return; }
    setSaving(true);
    try {
      await workflowAPI.update(id, { name: form.name, description: form.description, version: form.version, status: form.status, tags: form.tags.split(',').map(t => t.trim()).filter(Boolean), inputSchema });
  ...
  return (
    <div style={{ padding: '32px 40px', maxWidth: 680, margin: '0 auto' }}>
      <div style={{ marginBottom: 28 }}>
        <Link to={`/workflows/${id}`} style={{ color: '#64748b', textDecoration: 'none', fontSize: 13 }}>← Back to Workflow</Link>
        <h1 style={{ fontSize: 26, fontWeight: 700, color: '#f1f5f9', margin: '8px 0 4px' }}>Edit Workflow</h1>
      </div>
      <form onSubmit={handleSubmit}>
        <div style={{ background: '#0f172a', border: '1px solid #1e293b', borderRadius: 12, padding: 28, marginBottom: 20 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            <div style={fieldStyle}>
              ...
              <input style={inputStyle} value={form.version} onChange={e => set('version', e.target.value)} required />
            </div>
            <div style={fieldStyle}>
        ...
        <div style={{ background: '#0f172a', border: '1px solid #1e293b', borderRadius: 12, padding: 28, marginBottom: 24 }}>
          ...
          <textarea style={{ ...inputStyle, minHeight: 160, fontFamily: 'JetBrains Mono, monospace', fontSize: 13 }} value={form.inputSchema} onChange={e => { set('inputSchema', e.target.value); setSchemaError(''); }} />
          {schemaError && <div style={{ color: '#f87171', fontSize: 13, marginTop: 8 }}>{schemaError}</div>}
        </div>
        <div style={{ display: 'flex', gap: 12 }}>
          <button type="submit" disabled={saving} style={{ background: '#4f46e5', color: 'white', padding: '10px 24px', borderRadius: 8, border: 'none', cursor: saving ? 'not-allowed' : 'pointer', fontSize: 14, fontWeight: 500 }}>
            {saving ? 'Saving...' : 'Save Changes'}
          </button>
          <Link to={`/workflows/${id}`} style={{ background: '#1e293b', color: '#94a3b8', padding: '10px 24px', borderRadius: 8, textDecoration: 'none', fontSize: 14, fontWeight: 500, display: 'inline-flex', alignItems: 'center' }}>Cancel</Link>
        </div>
      </form>
    </div>
  );
